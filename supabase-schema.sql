...
alter table public.performances
drop constraint if exists performances_reference_url_check;

alter table public.performances
add constraint performances_reference_url_check check (instagram_url ~ '^https://((www\.)?instagram\.com|((www|m)\.)?youtube\.com|youtu\.be)/.+');
